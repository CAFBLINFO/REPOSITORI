...
Function GetUserPhoto(UserPhoto)
	...
	GetUserPhoto=""
	str_user_photo = " select * from [CD_Users_Mails] where [User]='" & UserPhoto & "'"
	set rs_user_photo = GetRecordset(str_user_photo)
	if not rs_user_photo.eof and not rs_user_photo.bof then
		GetUserPhoto = "" & rs_user_photo("Foto")
	end if
	rs_user_photo.close
	set rs_user_photo=Nothing	
